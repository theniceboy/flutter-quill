  ...
  group('paste', () {
    test('Plain', () async {
      final controller = QuillController.basic()
        ..compose(
            Delta()..insert('[]'), const TextSelection.collapsed(offset: 0), ChangeSource.local)
    ...
    test('Paste from external', () async {
      final source = QuillController.basic()
        ..compose(Delta()..insert('Plain text'), const TextSelection.collapsed(offset: 0),
            ChangeSource.local)
      ...
      final controller = QuillController.basic()
        ...
        ..updateSelection(const TextSelection.collapsed(offset: 1), ChangeSource.local);
      //
      expect(controller.pasteUsingPlainOrDelta('insert'), true, reason: 'External paste');
      expect(controller.document.toPlainText(), '[insert]\n');
    });
    ...
    test('Delta simple', () async {
      ...
      final controller = QuillController.basic()
        ...
        ..updateSelection(const TextSelection.collapsed(offset: 1), ChangeSource.local);
      //
      expect(controller.pasteUsingPlainOrDelta('n te'), true, reason: 'Internal paste');
    ...
    test('Delta multi line', () async {
      ...
      final source = QuillController.basic()
        ..compose(Delta()..insert('ABC\nDEF\nGHI\nJKL'), const TextSelection.collapsed(offset: 0),
            ChangeSource.local)
      ...
      final controller = QuillController.basic()
        ...
        ..updateSelection(const TextSelection.collapsed(offset: 1), ChangeSource.local);
      //
      expect(controller.pasteUsingPlainOrDelta(plainSelection), true, reason: 'Internal paste');
