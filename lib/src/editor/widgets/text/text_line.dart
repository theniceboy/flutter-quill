...
typedef LinkRecognizerGetter = GestureRecognizer? Function(Node segment, bool isLink);
...
class TextLineState extends State<TextLine> {
  ...
  static bool isPlaceholderLine(TextLine widget) =>
      widget.line.toDelta().first.attributes?.containsKey('placeholder') ?? false;
  ...
  static InlineSpan getTextSpanForWholeLine(
      TextLine widget, BuildContext context, LinkRecognizerGetter? getRecognizer) {
    var lineStyle = _getLineStyle(widget, widget.styles);
    if (!widget.line.hasEmbed) {
      return _buildTextSpan(widget, widget.styles, widget.line.children, lineStyle, getRecognizer);
    }
    ...
    for (var child in widget.line.children) {
      if (child is Embed) {
        if (textNodes.isNotEmpty) {
          textSpanChildren
              .add(_buildTextSpan(widget, widget.styles, textNodes, lineStyle, getRecognizer));
  ...
  static TextSpan _buildTextSpan(
    ...
    if (isComposingRangeOutOfLine) {
      final children = nodes
          .map((node) =>
              _getTextSpanFromNode(widget, defaultStyles, node, widget.line.style, getRecognizer))
  ...
  static List<InlineSpan> _splitAndApplyComposingStyle(
    ...
    final composingStyle =
        child.style?.merge(const TextStyle(decoration: TextDecoration.underline)) ??
  ...
  static TextStyle _getLineStyle(TextLine widget, DefaultStyles defaultStyles) {
    ...
    textStyle = _applyCustomAttributes(widget, textStyle, widget.line.style.attributes);
  ...
  static InlineSpan _scriptSpan(TextLine widget, String text, bool superScript, TextStyle style,
      DefaultStyles defaultStyles) {
    ...
    final fontWeight = FontWeight.lerp(
        style.fontWeight ?? lineStyle?.fontWeight ?? FontWeight.normal, FontWeight.w900, 0.25);
    final fontSize = style.fontSize ?? lineStyle?.fontSize ?? 16;
    final y = (superScript ? -0.4 : 0.14) * fontSize;
    final charStyle = style
        .copyWith(fontFeatures: <FontFeature>[], fontWeight: fontWeight, fontSize: fontSize * 0.7);
    //
  ...
  static InlineSpan _getTextSpanFromNode(TextLine widget, DefaultStyles defaultStyles, Node node,
      Style lineStyle, LinkRecognizerGetter? getRecognizer) {
    final textNode = node as leaf.QuillText;
    final nodeStyle = textNode.style;
    final isLink = nodeStyle.containsKey(Attribute.link.key) &&
        nodeStyle.attributes[Attribute.link.key]!.value != null;
    final style = getInlineTextStyle(widget, nodeStyle, defaultStyles, lineStyle, isLink);
    if (widget.controller.configurations.requireScriptFontFeatures == false &&
      ...
      if (nodeStyle.containsKey(Attribute.script.key)) {
        ...
        if (attr == Attribute.superscript || attr == Attribute.subscript) {
          return _scriptSpan(
              widget, textNode.value, attr == Attribute.superscript, style, defaultStyles);
        }
      }
    }
  ...
  static TextStyle getInlineTextStyle(
      TextLine widget, Style nodeStyle, DefaultStyles defaultStyles, Style lineStyle, bool isLink) {
...
class EditableTextLine extends RenderObjectWidget {
  ...
  RenderObject createRenderObject(BuildContext context) {
    return RenderEditableTextLine(line, textDirection, textSelection, enableInteractiveSelection,
        hasFocus, devicePixelRatio, _getPadding(), color, cursorCont, inlineCodeStyle);
  }
...
class RenderEditableTextLine extends RenderEditableBox {
  ...
  TextPosition? getPositionAbove(TextPosition position) {
    double? maxOffset;
    double limit() =>
        maxOffset ??= _body!.semanticBounds.height / preferredLineHeight(position) + 1;
