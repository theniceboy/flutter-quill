...
void main() {
  ...
  group('controller', () {
    test('set document', () {
      ...
      final newDocument = Document.fromDelta(Delta()..insert(replacementContents));
    ...
    test('getSelectionStyle', () {
      controller
        ...
        ..updateSelection(const TextSelection(baseOffset: 0, extentOffset: 4), ChangeSource.local);
    ...
    test('indentSelection with multiline document', () {
      ...
      controller
        ..updateSelection(const TextSelection(baseOffset: 0, extentOffset: 11), ChangeSource.local)
        ..indentSelection(true);
      ...
      controller.updateSelection(
          TextSelection(baseOffset: 12, extentOffset: controller.document.toPlainText().length - 1),
          ChangeSource.local);
    ...
    test('getAllSelectionStyles', () {
      ...
      expect(controller.getAllSelectionStyles(), contains(const Style().put(Attribute.bold)));
    });

    test('undo', () {
      var listenerCalled = false;
      controller.updateSelection(const TextSelection.collapsed(offset: 4), ChangeSource.local);
    ...
    test('formatTextStyle', () {
      ...
      expect(controller.document.collectAllStyles(2, 4), everyElement(const Style()));
    });
    ...
    test('formatText', () {
      ...
      expect(listenerCalled, isTrue);
      expect(
          controller.document.collectAllStyles(0, 2), contains(const Style().put(Attribute.bold)));
      expect(controller.document.collectAllStyles(2, 4), everyElement(const Style()));
    });
    ...
    test('formatSelection', () {
      var listenerCalled = false;
      controller
        ..updateSelection(const TextSelection(baseOffset: 0, extentOffset: 2), ChangeSource.local)
        ..addListener(() {
          listenerCalled = true;
        })
    ...
    test('moveCursorToStart', () {
      var listenerCalled = false;
      controller
        ..updateSelection(const TextSelection.collapsed(offset: 4), ChangeSource.local)
        ..addListener(() {
          listenerCalled = true;
        });
    ...
    test('moveCursorToEnd', () {
      ...
      expect(listenerCalled, isTrue);
      expect(controller.selection, TextSelection.collapsed(offset: controller.document.length - 1));
    });
    ...
    test('blockSelectionStyles', () {
      Style select(int start, int end) {
        controller.updateSelection(
            TextSelection(baseOffset: start, extentOffset: end), ChangeSource.local);
      ...
      for (final blockKey in Attribute.blockKeys) {
        ...
        controller
          ...
          ..formatText(4, 6, Attribute.bold) // spans end of line 1 and start of line 2
          ..formatText(7, 0, blockAttribute);

        expect(select(2, 5), const Style().put(blockAttribute),
            reason: 'line 1 block, plain and bold');
        expect(select(5, 6), const Style().put(Attribute.bold).put(blockAttribute),
            reason: 'line 1 block, bold');
        expect(select(4, 8), const Style().put(Attribute.bold).put(blockAttribute),
        ...
        expect(select(7, 8), const Style().put(Attribute.bold).put(blockAttribute),
