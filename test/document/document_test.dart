...
void main() {
  group('collectStyle', () {
    test('No selection', () {
      ...
      //
      expect(document.getPlainText(0, document.length), 'plain\nbold\nitalic\n');
      ...
      for (var index = 6; index < 11; index++) {
        expect(const Style.attr({'bold': Attribute.bold}), document.collectStyle(index, 0));
      }
      //
      for (var index = 11; index < document.length; index++) {
        expect(const Style.attr({'italic': Attribute.italic}), document.collectStyle(index, 0));
      }
    });
    ...
    test('Change block value type', () {
      void doTest(Map<String, dynamic> start, Attribute attr) {
        ...
        expect(document.collectStyle(4, 0), Style.attr({'bold': Attribute.bold, attr.key: attr}),
    ...
    test('Simulate double enter key at end', () {
      ...
      final document = Document.fromDelta(delta);
      //
      expect(document.getPlainText(0, document.length), 'data\nsecond\n\n\nplain\n');
    ...
    test('Selection', () {
      ...
      //
      expect(const Style.attr({'bold': Attribute.bold}), document.collectStyle(5, 3));
      expect(const Style.attr({'bold': Attribute.bold}), document.collectStyle(8, 3));
      //
    ...
    test('Links and line boundaries', () {
      ...
      final document = Document.fromDelta(delta);
      //
      const linkStyle = Style.attr({'link': LinkAttribute('https://unknown.com')});
      //
      expect(document.collectStyle(15, 0), linkStyle, reason: 'Within Link');
      expect(document.collectStyle(16, 0), const Style(), reason: 'At end of link');
      expect(document.collectStyle(17, 0), const Style(), reason: 'start of blank line');
      expect(document.collectStyle(18, 0), const Style(), reason: 'start of blank line');
    });
  });
}
